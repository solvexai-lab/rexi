/**
 * Health Insurance Compare Page
 * Route: /insurance/health/compare
 *
 * Mirrors /insurance/compare (motor) exactly — same stage machine, same UI components.
 * Only differences:
 * 1. Endpoints: health-compare-extract + health-compare-analyse
 * 2. SESSION_KEY: rexi_health_compare_result (isolated from motor)
 * 3. Copy references "health policies" not "motor policies"
 * 4. Back link → /insurance/health
 * 5. Accent blob colour: emerald instead of blue
 *
 * All 5 result components (CompareWinners, CompareTable, etc.) are re-used as-is
 * because HealthCompareResult has the same structural shape as InsuranceCompareResult.
 */
"use client";

import { useState, useCallback, useEffect } from "react";
import Link from "next/link";
import { ArrowLeft, GitCompareArrows, HeartPulse, Loader2, RotateCcw } from "lucide-react";
...
/** Isolated from motor compare session */
const SESSION_KEY = "rexi_health_compare_result";

export default function HealthComparePage() {
    const [files, setFiles] = useState<FileEntry[]>([]);
    const [stage, setStage] = useState<Stage>("upload");
    const [progressEntries, setProgressEntries] = useState<ProgressEntry[]>([]);
    const [extractedInsurers, setExtractedInsurers] = useState<ExtractedHealthInsurer[]>([]);
    const [compareResult, setCompareResult] = useState<HealthCompareResult | null>(null);
    const [warning, setWarning] = useState<string | null>(null);
    const [error, setError] = useState<string | null>(null);

    // Restore results on mount (back-button safe)
    useEffect(() => {
        try {
    ...
    const startCompare = async () => {
        if (files.length < 2) {
            setError("Please upload at least 2 health insurance documents.");
            return;
        }
        ...
        // ── Phase 1: SSE health extraction ─────────────────────────────────────────
        const formData = new FormData();
        files.forEach(({ file }, i) => formData.append(`file${i}`, file));

        let extracted: ExtractedHealthInsurer[] = [];

        try {
            const res = await fetch("/api/insurance/health-compare-extract", {
                method: "POST",
                body: formData,
            });
        ...
        const validExtracted = extracted.filter((e: any) => !e.extractionError);
        ...
        // ── Phase 2: Gemini health comparison ──────────────────────────────────────
        setStage("analysing");

        try {
            const res2 = await fetch("/api/insurance/health-compare-analyse", {
                method: "POST",
            ...
            sessionStorage.setItem(
                SESSION_KEY,
                JSON.stringify({ result: data, insurers: validExtracted })
            );
            setStage("results");
        } catch (err: any) {
            setStage("upload");
    ...
    return (
        <div className="min-h-screen mesh-gradient px-4 py-10 md:py-20 relative overflow-hidden">
            {/* Background blobs — emerald instead of blue */}
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-emerald-100/30 rounded-full blur-[100px] -z-10 animate-pulse" />
            ...
            <div className="max-w-4xl mx-auto space-y-10">
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        Back to Health Studio
                    </Link>

                    <div className="flex items-center justify-between flex-wrap gap-3">
                        <div>
                            {/* Domain badge */}
                            <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-bold uppercase tracking-[0.2em] px-3 py-1.5 rounded-full mb-3">
                ...
                {stage === "upload" && (
                    <div className="glass-panel-heavy squircle-soft shadow-dreamy p-6 md:p-10 space-y-6">
                        ...
                        >
                            ...
                            Compare{" "}
                            {files.length > 0 ? `${files.length} Health Policies` : "Health Policies"}
                        </button>
                    </div>
                )}
                    ...
                    <div className="glass-panel-heavy rounded-3xl p-10 shadow-dreamy flex flex-col items-center gap-5">
                        ...
                        <div className="text-center">
                            <h3 className="font-serif font-bold text-2xl text-slate-950">
                                Running Health Comparison…
                            </h3>
                            <p className="text-slate-500 text-sm mt-2">
                                Comparing room rent rules, waiting periods, co-pays, and claim scenarios
                            </p>
                        </div>
                    </div>
                )}
                ...
                {stage === "results" && compareResult && (
                    <div className="space-y-8">
                        ...
                        {/* Re-use motor compare result components — same shape */}
                        <CompareWinners
                            winners={compareResult.winners as any}
                            insurers={extractedInsurers as any}
                        />
                        <CompareTable
                            rows={compareResult.comparisonRows as any}
                            insurers={extractedInsurers as any}
                        />
                        <CompareRiskMatrix
                            riskMatrix={compareResult.riskMatrix as any}
                            insurers={extractedInsurers as any}
                        />
                        <CompareClaimScenarios
                            scenarios={compareResult.claimScenarios as any}
                            insurers={extractedInsurers as any}
                        />
